# in this project classifying weapon images using SVM and CNN algorithms and GLCM features
...
labels = ['Bazooka', 'Grenade_Launcher', 'Handgun', 'Knife','Rifle','Shotgun',
          'Snipper','Sword']
...
main = tkinter.Tk()
main.title("Weapon_Detection with Deep Learning") #designing main screen
...
def uploadDataset():
    ...
    text.insert(END,"Different Weapon Found in Dataset : "+str(labels)+"\n\n") 
    text.insert(END,"Total Weapon Groups are : "+str(len(labels)))


def voice_alert(weapon_label):
    engine = pyttsx3.init()
    engine.say(f"Be alert! {weapon_label} is detected.")
    engine.runAndWait()
...
def predict():
    ...
    cv2.putText(img, 'Weapon Predicted as : '+labels[predict], (10, 25),  cv2.FONT_HERSHEY_SIMPLEX,0.7, (255, 0, 0), 2)
    cv2.imshow('Weapon Predicted as : '+labels[predict], img)
    cv2.waitKey(0)'''
...
def predict():
    ...
    weapon_label = labels[predict]

    img = cv2.imread(filename)
    img = cv2.resize(img, (800,400))
    cv2.putText(img, f'Weapon Predicted as: {weapon_label}', (10, 25),  cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
    cv2.imshow(f'Weapon Predicted as: {weapon_label}', img)
    cv2.waitKey(0)

    # Display text alert
    text.insert(END, f'Weapon Predicted as: {weapon_label}\n')

    # Call the voice alert function
    voice_alert(weapon_label)

font = ('times', 16, 'bold')
title = Label(main, text='Weapon_detection with Deep Learning ')
...
cnnButton = Button(main, text="Run Proposed CNN Algorithm", command=runCNN)
...
predictButton = Button(main, text="Weapon Detection from Test Image", command=predict)
predictButton.place(x=750,y=550)
